      ...
      <div className="glass-panel rounded-3xl p-6 sm:p-7 shadow-2xl space-y-6 border border-white/10 relative overflow-hidden" dir="rtl">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-800/80">
          ...
          <div className="flex flex-wrap items-center gap-2 self-start sm:self-auto">
            {/* Direct Booking & Payment CTA in Header */}
            {totalEstimated !== null && (
              <button
                type="button"
                onClick={() => setIsPaymentModalOpen(true)}
                className="btn-mint flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold shadow-lg hover:scale-105 transition animate-fade-in"
              >
                <CreditCard className="w-3.5 h-3.5" />
                <span>המשך לתשלום והזמנה ({formatPrice(totalEstimated)})</span>
              </button>
            )}
        ...
        {/* Metrics Banner with Dynamic Currency */}
        {totalEstimated !== null && (
        ...
        {activeTab === "structured" ? (
          <div className="space-y-6">
            ...
            <div className="p-6 rounded-2xl bg-gradient-to-r from-mint-500/10 via-indigo-500/10 to-transparent border border-mint-500/30 flex flex-col sm:flex-row items-center justify-between gap-4">
              <div className="space-y-1 text-right">
                <div className="flex items-center gap-2 text-mint-400 text-xs font-bold">
                  <Lock className="w-4 h-4" />
                  <span>הבטחת מחיר ותשלום מאובטח</span>
                </div>
                <div className="text-white font-bold text-base">
                  מוכן לצאת להרפתקה ב-{destination}?
                </div>
                <div className="text-xs text-slate-300">
                  שריין את כל הטיסות והמלונות במחיר של{" "}
